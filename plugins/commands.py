...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('✇ Jᴏɪɴ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ ✇', url='https://t.me/Tamil_Rockerz_net')
                  ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAII5GdwK1CRYIrkjs412YaDsHqtshVvAAJZAwACKBEJVZp384ZMGRJAHgQ") 
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            files_ = await get_file_details(file_id)
            files1 = files_[0]
            title = '@TamillRockerznet - - '.join(filter(lambda x: not x.startswith('[') and not x.startswith('€'), files1.file_name.split()))
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    f_caption=f_caption
            if f_caption is None:
                f_caption = f"{'@TamillRockerznet - - '.join(filter(lambda x: not x.startswith('[') and not x.startswith('€'), files1.file_name.split()))}"
    ...
    if not files_:
        ...
        try:
            ...
            title = '@TamillRockerznet - - ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('€'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@TamillRockerznet - - ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('€'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@TamillRockerznet - -{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('€'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
...
async def stop_button(bot, message):
    ...
    await msg.edit("**✅️ 𝙱𝙾𝚃 AJɪᴛʜ 𝙸𝚂 𝚁𝙴𝚂𝚃𝙰𝚁𝚃𝙴𝙳. 𝙽𝙾𝚆 𝚈𝙾𝚄 𝙲𝙰𝙽 𝚄𝚂𝙴 𝙼𝙴**")
